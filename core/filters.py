...
class FilterManager:
    ...
    async def apply_all_filters(self, symbol: str, signal: Dict, 
        ...
        if 'structure' in enabled_manual:
            ...
            if results['structure']:
                ...
                block_reason = "Market Structure mismatch"
                logger.info(f"   ❌ {symbol}: Filter STRUCTURE failed - {signal['direction']} not aligned with structure")
        ...
        if 'volume' in enabled_manual:
            ...
            if results['volume']:
                ...
                if not block_reason:
                    block_reason = "Volume below average"
                logger.info(f"   ❌ {symbol}: Filter VOLUME failed - insufficient volume")
        ...
        if 'liquidity' in enabled_manual:
            ...
            if results['liquidity']:
                ...
                if not block_reason:
                    block_reason = "Liquidity too low / Spread too high"
                logger.info(f"   ❌ {symbol}: Filter LIQUIDITY failed - spread too wide")
        ...
        if 'correlation' in enabled_manual:
            ...
            if results['correlation']:
                ...
                if not block_reason:
                    block_reason = "BTC correlation mismatch"
                logger.info(f"   ❌ {symbol}: Filter CORRELATION failed - moving against BTC")
        ...
        if 'funding' in enabled_manual:
            ...
            if results['funding']:
                ...
                logger.info(f"   ❌ {symbol}: Filter FUNDING failed - funding too expensive")
        ...
        if 'liquidation' in enabled_manual:
            ...
            if results['liquidation']:
                ...
                if not block_reason:
                    block_reason = "Liquidation risk high"
                logger.info(f"   ❌ {symbol}: Filter LIQUIDATION failed - near liquidation cluster")
        ...
        if 'mtf' in enabled_manual:
            ...
            if results['mtf']:
                ...
                if not block_reason:
                    block_reason = "Multi-timeframe not aligned"
                logger.info(f"   ❌ {symbol}: Filter MTF failed - 5m/15m/1h not confluent")
        ...
        if 'session' in enabled_manual:
            ...
            if results['session']:
                ...
                if not block_reason:
                    block_reason = "Not golden hour session"
                logger.info(f"   ❌ {symbol}: Filter SESSION failed - not optimal trading time")
        ...
        if features_df is not None and len(features_df) >= 60:
            ...
            if ml_dir:
                ...
                if not block_reason:
                    block_reason = "ML direction prediction negative"
                logger.info(f"   ❌ {symbol}: Filter ML_DIRECTION failed - AI predicts opposite")
            ...
            if ml_vol:
                ...
                if not block_reason:
                    block_reason = "ML volatility too high"
                logger.info(f"   ❌ {symbol}: Filter ML_VOLATILITY failed - market too choppy")
            ...
            if features_df is None:
                logger.info(f"   ⚠️ {symbol}: Features df is None, skipping ML filters")
            else:
                logger.info(f"   ⚠️ {symbol}: Insufficient data for ML ({len(features_df)} < 60)")

        # Check if blocked by critical filters
        blocked = False
        critical_filters = ['structure', 'liquidity', 'mtf']
        for cf in critical_filters:
            if cf in results and not results[cf]:
                blocked = True
                if not block_reason:
                    block_reason = f"Critical filter failed: {cf}"
    ...
    async def filter_liquidity(self, symbol: str, exchange_data: Dict) -> bool:
        ...
        if ask == 0 or bid == 0:
            logger.debug(f"{symbol}: Missing bid/ask data")
            return False
    ...
    async def filter_liquidation(self, symbol: str, signal: Dict, exchange_data: Dict) -> bool:
        ...
        # Placeholder - real implementation needs liq data
        return True
